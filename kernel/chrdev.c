...
static int mychrdev_open(struct inode* inode, struct file* file){
	...
	char *kbuf; kbuf = kmalloc(KBUF_SIZE, GFP_KERNEL);
	file->private_data = kbuf;
	printk(KERN_INFO "Opening device %s:\n\n", MYDEV_NAME);
	counter++;
	printk(KERN_INFO "counter: %d\n", counter);
	printk("module refcounter: %d\n", module_refcount(THIS_MODULE));
    return 0;
}

static int mychrdev_release(struct inode* inode, struct file* file){
	printk("Releasing device %s:\n\n", MYDEV_NAME);
	char *kbuf; kbuf = file->private_data;
...
static ssize_t mychrdev_read(struct file* file,
	...
	*ppos += nbytes;
	printk("Read device %s nbytes = %d, ppos = %d\n\n", MYDEV_NAME,
           nbytes, (int) *ppos);
	return nbytes;
}
...
static ssize_t mychrdev_write(struct file* file, 
	...
	*ppos += nbytes;
	printk("Write device %s nbytes = %d, ppos = %d\n\n", MYDEV_NAME,
            nbytes, (int) *ppos);
	return nbytes;
}
